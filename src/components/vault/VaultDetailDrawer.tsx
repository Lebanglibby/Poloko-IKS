      ...
      <div className="bg-white rounded-xl border border-[#BBF7D0] p-3">
        <div className="flex items-center justify-between mb-1">
          ...
          <button
            ...
            aria-label={copied ? 'Record identifier copied to clipboard' : 'Copy record identifier to clipboard'}
            aria-live="polite"
          >
            {copied
        ...
        <p
          className="font-mono text-[10px] text-[#4B5563] break-all leading-relaxed select-all"
          aria-label={`SHA-256 record identifier: ${hash}`}
        >
          {hash}
        </p>
      </div>
...
/* ─── Collapsible section ───────────────────────────────────────────────────── */
function Section({
  title, icon: Icon, defaultOpen = true, children, id: sectionId,
}: {
  title: string
  icon: typeof Shield
  defaultOpen?: boolean
  ...
  // Use provided id or derive from title for stable aria-controls
  const contentId = sectionId ?? `vault-sec-${title.replace(/\s+/g, '-').toLowerCase()}`
  ...
  return (
    <div className="rounded-2xl border border-[#E8DDD0] bg-white overflow-hidden">
      ...
      {open && (
        <div id={contentId} className="px-5 pb-5 space-y-3">
          {children}
        </div>
      )}
    </div>
    ...
    <div className="rounded-2xl border border-[#E8DDD0] overflow-hidden bg-[#FDFBF7]">
      <div className="relative aspect-video bg-[#F3F0EB] flex items-center justify-center">
        ...
        <img
          src={urls[active]}
          alt={urls.length > 1
            ? `Photo ${active + 1} of ${urls.length} for "${entryTitle}"`
            : `Photo for "${entryTitle}"`}
      ...
      {urls.length > 1 && (
        <div
          className="flex gap-2 p-3 border-t border-[#E8DDD0] overflow-x-auto"
          role="tablist"
        ...
        >
          {urls.map((u, i) => (
            <button
              ...
              onClick={() => setActive(i)}
              className={cn(
                'shrink-0 h-12 w-16 rounded-xl border-2 overflow-hidden transition-all',
...
export function VaultDetailDrawer({ entry, isOpen, onClose, userId, userRole }: Props) {
  ...
  // Ref for the live announcement region
  ...
  /* ── Loading shimmer on entry change + screen reader announcement ── */
  useEffect(() => {
    if (!entry) return
    setLoading(true)
    const t = setTimeout(() => setLoading(false), 200)
    // Announce new entry to screen readers after skeleton clears
    const announce = setTimeout(() => {
      if (announceRef.current && entry) {
        announceRef.current.textContent = `Loaded: ${entry.title}. ${KNOWLEDGE_CATEGORY_LABELS[entry.category]}.`
      }
    }, 280)
    return () => { clearTimeout(t); clearTimeout(announce) }
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className="fixed inset-0 z-[60] bg-[#1F2937]/40 backdrop-blur-[2px]"
        aria-hidden="true"
        onClick={onClose}
      />
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="drawer-entry-title"
        className={cn(
      ...
      >
        {/* Screen reader live region — announces when entry changes */}
        <p
          ref={announceRef}
          aria-live="polite"
          aria-atomic="true"
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E8DDD0] bg-white shrink-0">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 rounded-xl bg-[#FEF2E8] border border-[#FDBA74] flex items-center justify-center shrink-0">
              <FileText style={{ width: 18, height: 18 }} className="text-[#9A3412]" aria-hidden="true" />
            </div>
            <div>
              <p className="text-sm font-bold text-[#1F2937]">Knowledge Entry</p>
              <p className="text-xs text-[#9CA3AF]">
                <span lang="tn" title="Poloko IKS Heritage Archive">Poloko IKS</span>
                {' '}Heritage Archive
              </p>
            </div>
          </div>
          <button
            ...
            aria-label="Close entry details panel"
          >
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain">
          {loading || !entry ? (
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                <h2
                  id="drawer-entry-title"
                  className="text-xl font-bold text-[#1F2937] leading-snug mb-2"
                >
                  {entry.title}
                </h2>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {/* Language badge */}
                  {entry.language && entry.language === 'tn' && (
                    <span
                      className="px-2.5 py-0.5 rounded-full bg-[#FEF2E8] text-[#9A3412] text-xs font-semibold border border-[#FDBA74]"
                      lang="tn"
                      title="Content recorded in Setswana"
                    >
                      Setswana
                    </span>
                  )}
                ...
                {entry.profiles?.full_name && (
                  <p className="text-sm text-[#9CA3AF] mt-2">
                    Submitted by{' '}
                    <span className="text-[#4B5563] font-medium">{entry.profiles.full_name}</span>
                    {entry.profiles.community && (
                      <> · <span lang="tn" className="text-[#4B5563]">{entry.profiles.community}</span></>
                    )}
                  </p>
                )}
              ...
              {canView && entry.description && (
                <Section title="About this entry" icon={FileText} id="sec-about">
                  <p
                    className="text-sm text-[#4B5563] leading-relaxed"
                    lang={entry.language === 'tn' ? 'tn' : 'en'}
                  >
                    {entry.description}
                  </p>
                </Section>
              ...
              {canView && entry.media_urls && entry.media_urls.length > 0 && (
                <Section title="Photos & Media" icon={ImageIcon} id="sec-media">
                  <MediaStrip urls={entry.media_urls} entryTitle={entry.title} />
                </Section>
              )}

              {/* ── Location ───────────────────────────────────────── */}
              {canView && entry.latitude && entry.longitude && (
                <Section title="Location" icon={MapPin} defaultOpen={false} id="sec-location">
                  <div className="flex items-center justify-between bg-[#FDFBF7] rounded-xl border border-[#E8DDD0] p-3 gap-3">
                    <p className="text-sm text-[#4B5563]">
                      {entry.access_tier === 'public'
                        ? `${entry.latitude.toFixed(4)}°S, ${entry.longitude.toFixed(4)}°E`
                        : 'Exact location is protected for this entry.'
                      }
                    </p>
                    <a
                      href={`/map?highlight=${entry.id}`}
                      className="shrink-0 text-sm font-semibold text-[#9A3412] hover:underline"
                      aria-label={`View ${entry.title} on the resource map`}
                    >
                      View on map →
                    </a>
                  </div>
              ...
              {canVerify && (
                <Section title="Help Verify This Entry" icon={CheckCircle2} defaultOpen={false} id="sec-verify">
                  <p className="text-sm text-[#4B5563] leading-relaxed mb-3">
                    As a community member, you can confirm the accuracy of this knowledge entry and
                    help build trust in the archive.
                  </p>
          ...
          <div className="shrink-0 border-t border-[#E8DDD0] bg-white px-6 py-4">
            <div className="grid grid-cols-2 gap-3">
              <a
                ...
                aria-label={`Open full page for: ${entry.title}`}
              >
