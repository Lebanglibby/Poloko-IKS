...
import {
  ...
  MapPin, FileText, Zap, Menu, X as XIcon,
  Sprout, ScrollText, TreePine, GraduationCap,
} from 'lucide-react'

/* ─── Decorative diamond separator ───────────────────────────────────────────── */
function DiamondDivider({ label }: { label: string }) {
  return (
    <div className="divider-ornament max-w-xs mx-auto my-4" aria-hidden="true">
      ◆ {label} ◆
    </div>
  )
}
...
function StatPill({ value, label }: { value: string; label: string }) {
  return (
    <div className="flex flex-col items-center px-6 py-4 rounded-2xl bg-white/10 border border-white/20 backdrop-blur-sm">
      <span className="font-display text-3xl font-bold text-white leading-none" aria-label={`${value} ${label}`}>
        {value}
      </span>
      <span className="text-sm text-orange-200 mt-1 font-medium" aria-hidden="true">{label}</span>
    </div>
  )
}
...
/* ─── Sample vault entry teaser ──────────────────────────────────────────────── */
interface TeaserEntry {
  category: string
  categoryColor: string
  categoryBg: string
  tier: 'Public' | 'Restricted' | 'Sacred'
  tierColor: string
  tierBg: string
  title: string
  community: string
...
const TEASER_ENTRIES: TeaserEntry[] = [
  {
    category: 'Medicinal Plants',
    categoryColor: 'text-green-800',
    categoryBg: 'bg-green-50 border-green-200',
    tier: 'Public',
    tierColor: 'text-[#15803D]',
    tierBg: 'bg-[#F0FDF4] border-[#BBF7D0]',
    title: 'Healing Properties of Morama Bean (Tylosema esculentum)',
    community: 'Kgalagadi Communities',
    excerpt: 'The morama bean has been used for centuries as a drought-resistant food source and medicinal plant. Communities record its preparation for treating digestive ailments and its nutritional properties during dry seasons.',
    tags: ['morama', 'Kgalagadi', 'medicinal', 'drought-resistant'],
    verified: true,
  },
  {
    ...
    tierBg: 'bg-[#FFFBEB] border-[#FDE68A]',
    title: 'Mophane Worm Harvesting Seasonal Cycle',
    community: 'Ngamiland District',
    excerpt: 'Seasonal harvesting practices for Gonimbrasia belina (mophane worm) passed down through generations. Documents timing, preservation methods, and the community protocols governing sustainable harvest.',
    tags: ['mophane', 'food-systems', 'seasonal', 'preservation'],
    verified: true,
  },
  {
    category: 'Conservation',
    categoryColor: 'text-teal-800',
    categoryBg: 'bg-teal-50 border-teal-200',
    tier: 'Public',
    tierColor: 'text-[#15803D]',
    tierBg: 'bg-[#F0FDF4] border-[#BBF7D0]',
    title: 'Okavango Delta Water Management Wisdom',
    community: 'BaYei People',
    excerpt: 'Traditional flood-channel management and seasonal migration patterns of the BaYei people of the Okavango Delta. Knowledge includes reading water signs, fish migration patterns, and sacred water sites.',
    tags: ['Okavango', 'water', 'BaYei', 'conservation'],
    verified: false,
  },
]

/* ─── Vault teaser card ──────────────────────────────────────────────────────── */
function VaultTeaserCard({ entry, index }: { entry: TeaserEntry; index: number }) {
  const delay = ['fade-up-delay-1', 'fade-up-delay-2', 'fade-up-delay-3'][index] ?? ''
  const tierIcon = entry.tier === 'Public' ? Globe : entry.tier === 'Restricted' ? Lock : Shield
  const TierIcon = tierIcon

  return (
    <article
      className={`group bg-white rounded-2xl border border-[#E8DDD0] overflow-hidden card-lift fade-up ${delay}`}
      aria-label={`${entry.title} — ${entry.category}`}
    >
      {/* Top accent strip */}
      <div
        className={`h-1.5 w-full ${
          entry.tier === 'Sacred' ? 'bg-[#B91C1C]' :
          entry.tier === 'Restricted' ? 'bg-[#B45309]' :
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
          <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border ${entry.categoryBg} ${entry.categoryColor}`}>
            {entry.category}
          </span>
          <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold border ${entry.tierBg} ${entry.tierColor}`}>
        ...
        {/* Title */}
        <h3 className="font-bold text-[#1F2937] text-base leading-snug mb-1 group-hover:text-[#9A3412] transition-colors">
          {entry.title}
        </h3>
        <p className="text-xs text-[#9CA3AF] mb-3">{entry.community}</p>

        {/* Excerpt */}
        <p className="text-sm text-[#4B5563] leading-relaxed line-clamp-3 mb-4">
          {entry.excerpt}
        </p>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4" aria-label="Tags">
          {entry.tags.slice(0, 3).map(t => (
            <span key={t} className="text-xs px-2 py-0.5 bg-[#F3F0EB] text-[#6B5344] rounded-full">
              {t}
            </span>
          ))}
        </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-[#F3F0EB]">
          {entry.verified ? (
            ...
            <span className="text-xs text-[#B45309] font-medium">Awaiting verification</span>
          )}
          <span className="text-xs font-semibold text-[#9CA3AF] group-hover:text-[#9A3412] flex items-center gap-1 transition-colors">
            View entry
  ...
  return (
    <div className="min-h-screen bg-[#FDFAF5]">
      ...
      <nav
        ...
        className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-[#E5D8C8]"
      ...
      >
        <div className="max-w-screen-xl mx-auto px-4 sm:px-8 h-[68px] flex items-center justify-between gap-6">
          ...
          {/* Logo */}
          <Link href="/" className="flex items-center gap-3 group shrink-0" aria-label="Poloko IKS — go to home page">
            <div className="relative flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-[#8B2500] to-[#C44B1A] shadow-md group-hover:shadow-lg transition-shadow">
              ...
              <div className="absolute inset-0 rounded-xl ring-2 ring-white/20" aria-hidden="true" />
            </div>
          ...
          {/* Desktop links */}
          <div className="hidden md:flex items-center gap-0.5 flex-1 justify-center" role="list">
          ...
          <div className="hidden sm:flex items-center gap-2 shrink-0">
            <Link href="/login"
              className="flex items-center px-4 py-2 rounded-xl text-sm font-semibold text-[#6B5344] border border-[#E5D8C8] hover:border-[#8B2500] hover:text-[#8B2500] hover:bg-[#FEF0E6] transition-all">
              Sign In
            </Link>
            <Link href="/register"
          ...
          <button
            ...
            className="md:hidden flex items-center justify-center h-10 w-10 rounded-xl border border-[#E5D8C8] text-[#6B5344] hover:bg-[#FEF0E6] hover:text-[#8B2500] transition-all"
        ...
        {mobileNavOpen && (
          <div
            id="mobile-nav"
            className="md:hidden mobile-nav-enter border-t border-[#E5D8C8] bg-white px-4 pb-5 pt-3"
          >
                ...
                {NAV_LINKS.map(({ href, label, icon: Icon }) => (
                  <li key={href}>
                    <Link
                      ...
                      className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold text-[#6B5344] hover:text-[#8B2500] hover:bg-[#FEF0E6] transition-all"
                    >
                      <Icon className="h-4 w-4 text-[#8B2500]" aria-hidden="true" />
                      {label}
                    </Link>
                  </li>
                ))}
              </ul>
              <div className="grid grid-cols-2 gap-2 border-t border-[#F3F0EB] pt-4">
                <Link
                  href="/login"
                  onClick={() => setMobileNavOpen(false)}
                  className="flex items-center justify-center py-2.5 rounded-xl text-sm font-semibold text-[#6B5344] border border-[#E5D8C8] hover:border-[#8B2500] hover:text-[#8B2500] transition-all"
                >
                  Sign In
                </Link>
                <Link
      ...
      <main id="main-content">
        <section
          aria-label="Hero — Preserving Botswana's Living Heritage"
        ...
        >
          ...
          <div className="absolute top-0 left-0 right-0 h-1.5 kente-stripe kente-stripe-animate" aria-hidden="true" />

          <div className="relative z-10 max-w-screen-xl mx-auto px-4 sm:px-8 pt-24 pb-20 sm:pt-32 sm:pb-28 text-center">

            {/* Location badge */}
            <div
              className="inline-flex items-center gap-2 bg-white/12 text-orange-100 text-xs sm:text-sm px-4 py-2 rounded-full mb-8 border border-white/20 backdrop-blur-sm fade-up"
              aria-label="Platform location: Botswana, Indigenous Knowledge Systems"
            >
            ...
            {/* Setswana meaning — language marked up */}
            <p className="text-lg text-orange-100/90 mb-3 font-medium fade-up fade-up-delay-2">
              <span lang="tn" className="font-display text-[#FFD580] italic">Poloko</span>
            ...
            <div className="flex flex-wrap gap-4 justify-center mb-16 fade-up fade-up-delay-3">
              <Link href="/vault"
                ...
                <BookOpen className="h-[18px] w-[18px] group-hover:scale-110 transition-transform" aria-hidden="true" />
                Browse the Archive
        ...
        {/* ════════════════════════════════════════
            TRUST STRIP
        ════════════════════════════════════════ */}
        <section aria-label="Platform trust signals" className="bg-white border-b border-[#E5D8C8] px-4 sm:px-8 py-5">
          <div className="max-w-screen-xl mx-auto">
            <ul className="flex flex-wrap items-center justify-center gap-5 sm:gap-10" role="list">
              {[
        ...
        {/* ════════════════════════════════════════
            FROM THE VAULT — Live preview
        ════════════════════════════════════════ */}
        <section aria-labelledby="vault-preview-heading" className="px-4 sm:px-8 py-20 bg-[#FDFAF5]">
          <div className="max-w-screen-xl mx-auto">
            <div className="text-center mb-14">
              <DiamondDivider label="From the Archive" />
              <h2
                id="vault-preview-heading"
                className="font-display text-4xl sm:text-5xl text-[#1A1008] mt-6 mb-4"
              >
                Knowledge Lives Here
              </h2>
              <p className="text-[#6B5344] text-lg max-w-xl mx-auto leading-relaxed">
                A glimpse into Botswana&apos;s heritage — real entries from the living archive,
                contributed by communities across the country.
              </p>
            </div>

            <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto mb-10">
              {TEASER_ENTRIES.map((entry, i) => (
                <VaultTeaserCard key={entry.title} entry={entry} index={i} />
              ))}
            </div>

            {/* Category quick-links */}
            <div className="flex flex-wrap justify-center gap-3 mb-10" role="list" aria-label="Browse by category">
              {[
                { icon: Sprout,    label: 'Medicinal Plants',     href: '/vault?category=flora_medicinal'      },
                { icon: ScrollText, label: 'Cultural Stories',    href: '/vault?category=cultural_narrative'   },
                { icon: TreePine,  label: 'Conservation Wisdom',  href: '/vault?category=conservation'         },
                { icon: Leaf,      label: 'Traditional Practices',href: '/vault?category=traditional_practice' },
                { icon: MapPin,    label: 'Resource Locations',   href: '/vault?category=resource_location'    },
              ].map(({ icon: Icon, label, href }) => (
                <Link
                  key={label}
                  href={href}
                  role="listitem"
                  className="flex items-center gap-2 px-4 py-2.5 rounded-full border-2 border-[#E5D8C8] bg-white text-sm font-semibold text-[#6B5344] hover:border-[#8B2500] hover:text-[#8B2500] hover:bg-[#FEF0E6] transition-all"
                >
                  <Icon className="h-3.5 w-3.5" aria-hidden="true" />
                  {label}
                </Link>
            ...
            <div className="text-center">
              <Link
                href="/vault"
                className="group inline-flex items-center gap-2.5 bg-[#8B2500] hover:bg-[#7C1D0E] text-white px-8 py-3.5 rounded-2xl font-bold text-sm sm:text-base transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5"
              >
                <BookOpen className="h-4.5 w-4.5" style={{ width: 18, height: 18 }} aria-hidden="true" />
                Browse All {' '}
                <span lang="tn" className="italic text-orange-200">Kitso</span>
                {' '} Entries
                <ArrowRight className="h-4 w-4 group-hover:translate-x-0.5 transition-transform" aria-hidden="true" />
              </Link>
            </div>
        ...
        {/* ════════════════════════════════════════
            PROBLEM STATEMENT
        ════════════════════════════════════════ */}
        <section aria-labelledby="problem-heading" className="px-4 sm:px-8 py-20 bg-white border-t-2 border-[#E5D8C8]">
          <div className="max-w-screen-xl mx-auto">
            <div className="text-center mb-14">
              <DiamondDivider label="Why This Matters" />
              <h2 id="problem-heading" className="font-display text-4xl sm:text-5xl text-[#1A1008] mt-6 mb-4">
            ...
            <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
              {[
                ...
                <div key={title}
                  ...
                  <div className="text-5xl mb-5 leading-none" role="img" aria-label={title}>{icon}</div>
        ...
        {/* ════════════════════════════════════════
            TWO PILLARS
        ════════════════════════════════════════ */}
        <section aria-labelledby="pillars-heading" className="px-4 sm:px-8 py-20 bg-[#FDFBF7] bg-pattern-lines border-y-2 border-[#E5D8C8]">
          <div className="max-w-screen-xl mx-auto">
            <div className="text-center mb-14">
              <DiamondDivider label="Three Ways to Contribute" />
              <h2 id="pillars-heading" className="font-display text-4xl sm:text-5xl text-[#1A1008] mt-6 mb-4">
                Built for Everyone
              </h2>
            ...
            <div className="grid lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
              ...
              <div className="group relative rounded-3xl overflow-hidden border-2 border-[#E5D8C8] bg-[#FDFAF5] hover:border-[#F4A878] transition-all hover:-translate-y-1"
                ...
                <div className="h-2 w-full" style={{ background: 'linear-gradient(90deg, #8B2500, #C44B1A)' }} aria-hidden="true" />
                <div className="p-8">
              ...
              <div className="group relative rounded-3xl overflow-hidden border-2 border-[#E5D8C8] bg-[#FDFAF5] hover:border-[#86EFAC] transition-all hover:-translate-y-1"
                ...
                <div className="h-2 w-full" style={{ background: 'linear-gradient(90deg, #166534, #16A34A)' }} aria-hidden="true" />
                <div className="p-8">
                  <div className="flex items-start gap-4 mb-6">
                    <div className="h-14 w-14 rounded-2xl flex items-center justify-center shrink-0 shadow-md"
              ...
              {/* Learning Hub card — Module 3 */}
              <div className="group relative rounded-3xl overflow-hidden border-2 border-[#E5D8C8] bg-[#FDFAF5] hover:border-[#FDE68A] transition-all hover:-translate-y-1"
                style={{ boxShadow: '0 4px 24px rgba(146,64,14,0.07)' }}>
                <div className="h-2 w-full" style={{ background: 'linear-gradient(90deg, #92400E, #B45309, #D97706)' }} aria-hidden="true" />
                <div className="p-8">
                  <div className="flex items-start gap-4 mb-6">
                    <div className="h-14 w-14 rounded-2xl flex items-center justify-center shrink-0 shadow-md"
                    ...
                    <div>
                      <h3 className="font-display text-2xl text-[#1A1008]">Learning Hub</h3>
                      <p className="text-sm text-[#9C8070] mt-0.5 font-medium">
                        <span lang="tn" className="font-semibold">Thuto</span> — Courses &amp; skills
                      </p>
                    </div>
                  </div>
                  <ul className="space-y-3.5 mb-8">
                    {[
                      { icon: GraduationCap, text: 'Learn basket weaving, natural cooking & cultural arts'   },
                      { icon: CheckCircle2,  text: 'Every course Elder Board approved before going live'      },
                      { icon: Users,         text: 'Taught by verified community knowledge holders'           },
                      { icon: Zap,           text: 'Free and paid courses — creators earn in BWP'            },
                      { icon: Shield,        text: 'Cultural sovereignty protected throughout'                },
                    ].map(item => <FeatureItem key={item.text} {...item} />)}
                  </ul>
                  <Link href="/learn"
                    className="group/btn inline-flex items-center gap-2 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-sm hover:shadow-md"
        ...
        {/* ════════════════════════════════════════
            ACCESS TIERS
        ════════════════════════════════════════ */}
        <section aria-labelledby="tiers-heading" className="px-4 sm:px-8 py-20 bg-white">
          <div className="max-w-screen-xl mx-auto">
            <div className="text-center mb-14">
              <DiamondDivider label="Community Sovereignty" />
              <h2 id="tiers-heading" className="font-display text-4xl sm:text-5xl text-[#1A1008] mt-6 mb-4">
        ...
        {/* ════════════════════════════════════════
            SOVEREIGNTY & TRUST
        ════════════════════════════════════════ */}
        <section aria-labelledby="trust-heading" className="px-4 sm:px-8 py-20 bg-[#FDFAF5] border-t-2 border-[#E5D8C8]">
          <div className="max-w-screen-xl mx-auto">
            <div className="text-center mb-14">
              <DiamondDivider label="Built to Last" />
              <h2 id="trust-heading" className="font-display text-4xl sm:text-5xl text-[#1A1008] mt-6 mb-4">
                Sovereignty &amp; Trust
              </h2>
              <p className="text-[#6B5344] text-lg max-w-xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
              {[
                ...
                <div key={title}
                  className="group text-center rounded-3xl border-2 border-[#E5D8C8] bg-white p-8 transition-all hover:-translate-y-1 hover:border-[#F4A878]"
                  ...
                  <h3 className="font-display text-xl text-[#1A1008] mb-3">{title}</h3>
        ...
        <section
          ...
          className="relative overflow-hidden px-4 sm:px-8 py-20"
          style={{ background: 'linear-gradient(135deg, #5C1006 0%, #8B2500 50%, #92400E 100%)' }}
        >
          <div className="absolute inset-0 bg-pattern-dots" aria-hidden="true" />
          <div className="absolute bottom-0 left-0 right-0 h-1.5 kente-stripe" aria-hidden="true" />

          <div className="relative z-10 max-w-2xl mx-auto text-center">
            <div className="text-5xl mb-6" role="img" aria-label="Plant seedling">🌿</div>
            <h2 id="cta-heading" className="font-display text-4xl sm:text-5xl text-white mb-4 leading-tight">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              ...
              <Link href="/vault"
                className="flex items-center gap-2.5 border-2 border-white/30 text-white/80 px-8 py-4 rounded-2xl font-bold text-base hover:bg-white/10 hover:border-white/50 transition-all">
          ...
          <div className="max-w-screen-xl mx-auto">
            <div className="grid sm:grid-cols-12 gap-10 mb-12">
              ...
              <div className="sm:col-span-5">
                ...
                <p lang="tn" className="text-sm text-[#F4A878] font-medium mt-3 italic">
                  &ldquo;Go Boloka Kitso ya Setso&rdquo;
                </p>
                <p className="text-xs text-[#6B5344] mt-0.5">Preserving the knowledge of the ancestors</p>
                <div className="mt-5 divider-ornament max-w-[160px]" aria-hidden="true">
                  ◆ Botswana ◆
                </div>
              </div>

              {/* Links */}
              <nav aria-label="Footer explore links" className="sm:col-span-3">
              ...
              <nav aria-label="Footer get started links" className="sm:col-span-4">
                ...
                <ul className="space-y-2.5">
                  {[
                    ...
                    { href: '/create',    label: 'Create a Course'    },
